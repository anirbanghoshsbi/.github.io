...
conditions = [
    (data["dy_pct"] > 0.85) & (data["pe_spread"] < -0.20),
    (data["dy_pct"] > 0.75) & (data["pe_spread"] < -0.25) & (data['elder_impulse']==-1),
...
data.loc[data["valuation_regime"] == "value", "valuation_regime"].tail(50)

data.groupby("valuation_regime")["future_return"].mean()*100
mv nse_pe.py.txt nse_pe.py
